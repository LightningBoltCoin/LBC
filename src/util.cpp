...
const char * const BITCOIN_CONF_FILENAME = "lightningboltcoin.conf";
const char * const BITCOIN_PID_FILENAME = "lightningboltcoin.pid";
...
#else
    const char* pszModule = "lightningboltcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lightningboltcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Lightningboltcoin";
#else
    // Unix
    return pathRet / ".lightningboltcoin";
#endif
#endif
}
